 ...
 * 
 */
class ExpoAction extends AdminAction {
...
/* ========用户部分======== */
    public function index(){
        ...
        $list = $UserDB->where($map)->order('start DESC')->page($nowPage.','.C('web_admin_pagenum'))->select();
    ...
    public function search(){
        if(isset($_POST['dosubmit'])) {
            ...
            if($_POST['time'])
            {
                $map['start'] = array('LT',$_POST['time']);
                $map['end'] = array('GT',$_POST['time']);
            }           

            $list = $UserDB->where($map)->order('start DESC')->select(); 
             $this->assign('list',$list);
             $this->display("index");
        }       
    }
    ...
    public function add(){
        ...
        if(isset($_POST['dosubmit'])) {
            ...
            if($ExpoDB->create()){
                if($ExpoDB->add()){
                    $this->assign("jumpUrl",U('/Admin/Expo/index'));
                    $this->success('添加成功!');
                }else{
                     $this->error('添加失败!');
                }
            }else{
                $this->error($ExpoDB->getError());
            }
        }else{
...
/* ========角色部分======== */
    ...
    public function del(){
        ...
        if($ExpoDB->where('id='.$id)->delete())
        {
             $this->assign("jumpUrl",U('/Admin/Expo/index',array('eid' =>$eid,'p'=>$nowPage)));
             $this->success('删除成功！');
        }else{
                $this->error('删除失败!');
        }      
    ...
    public function batchdel(){
        ...
        if($ExpoDB->where('id in ('.$ids.')')->delete())
        {
             $this->assign("jumpUrl",U('/Admin/Expo/index',array('eid' =>$eid,'p'=>$nowPage)));
             $this->success('删除成功！');
        }else{
    ...
    public function edit(){
        ...
        if(isset($_POST['dosubmit'])) {         
            ...
            $id = $this->_get('id','intval',0);
            if(!$id)$this->error('参数错误!');
            $info = $ExpoDB->getShow(array('id'=>$id));
        }
