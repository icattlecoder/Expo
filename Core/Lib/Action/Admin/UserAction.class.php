...
/* ========用户部分======== */
    public function index(){
        ...
        if(session('username')!=C('SPECIAL_USER')){     //无视系统权限的那个用户     
           $count = $UserDB->where('company='.session('company'))->count();
        }else{
        ...
        if(session('username') != C('SPECIAL_USER')){        
            $list = $UserDB->where('company='.session('company'))->order('id ASC')->page($nowPage.','.C('web_admin_pagenum'))->select();
