...
<div class="topnav">
  <div class="sitenav">
    <div class=welcome>您好：<span class="username"><?php echo (session('username')); ?> </span>，欢迎使用<?php echo (C("cms_name")); ?>！ </div>
    <div class=sitelink><a href="<?php echo U('Admin/Index/Index');?>" target="_parent">后台首页</a> | <a href="<?php echo (C("web_url")); ?>" target="_blank">网站主页</a> | <a href="<?php echo (C("cms_url")); ?>" target="_blank">官方网站</a> | <a class="top-txt" href="<?php echo U('Admin/Cache/delCore');?>" target="frmright" id="delcache">更新缓存</a><a id="cache"></a></div>
  </div>
  <div class="leftnav">
    <ul>
      <li class="navleft"></li>
<?php if(is_array($main_menu)): $i = 0; $__LIST__ = $main_menu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($i == 1){ $first_menu_id = $vo['id']; } ?>
	  <li id="d<?php echo ($i); ?>" <?php if(($i) == "1"): ?>class="thisclass"<?php endif; ?> onClick="ConClass(<?php echo ($i); ?>);"><a href="<?php echo U('/Admin/Index/left/id/'.$vo['id']);?>" target="leftfra" ><?php echo ($vo["title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
      <li id="logout" style="margin-right: -1px"><a href="<?php echo U('Admin/Login/logout');?>" target="_parent">注销登录</a></li>
...
<div id="Maincontent">
  ...
  <div class="mframe">
 <iframe name="main" id="main" src="<?php echo U('/Admin/Index/main');?>" frameborder="false" scrolling="auto" style="border:none; margin-bottom:10px;"  width="100%" height="auto" ></iframe>
  </div>
